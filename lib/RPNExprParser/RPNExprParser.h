...
public:
  CustomFunctionWrapper() : invalid{true} {}
  CustomFunctionWrapper(const std::function<T(const std::vector<T> &)> &_func,
                        size_t _argCnt = 0, bool _limitedArgc = false)
      : func{_func}, argCnt{_argCnt}, limitedArgc{_limitedArgc} {
    if (argCnt && !limitedArgc)
      throw std::runtime_error{"[FunctionCall] 函数参数限制初始化错误."};
    // std::cout << "ctor" << std::endl;
  }
  CustomFunctionWrapper(const CustomFunctionWrapper &obj) = default;
  CustomFunctionWrapper(const CustomFunctionWrapper &&obj) noexcept
      : func{std::move(obj.func)}, argCnt{obj.argCnt},
  ...
  T operator()(const std::vector<T> &args) const {
    if (limitedArgc && args.size() != argCnt)
      throw std::runtime_error{"[FunctionCall] 无效的函数调用, 参数不匹配."};
    return func(args);
  }
...
public:
  CustomOperatorWrapper() : invalid{true} {}
  CustomOperatorWrapper(const std::function<T(const T &, const T &)> &_func,
                        int _priority)
      : func{_func}, priority{_priority} {}
  CustomOperatorWrapper(const CustomOperatorWrapper &obj) = default;
  CustomOperatorWrapper(const CustomOperatorWrapper &&obj) noexcept
      : func{std::move(obj.func)}, priority{obj.priority} {}
  ~CustomOperatorWrapper() = default;

  T operator()(const T &a, const T &b) const { return func(a, b); }

  friend bool operator<(const CustomOperatorWrapper &l,
                        const CustomOperatorWrapper &r) {
...
template <typename T> using Functor = CustomFunctionWrapper<T>;
...
template <typename T,
          typename Requires =
              typename std::void_t<decltype(T(std::declval<double>()))>>
class RPNExprParser {
private:
...
protected:
  ...
  inline bool isDigit(char c) { return (('0' <= c && c <= '9') || c == '.'); }
  inline bool isDelim(char c) { return c == delim; }
  ...
  std::string parse(std::size_t beg, std::size_t end,
    ...
    while (l < end) {
      while (l < end && isDelim(originExpr[l]))
        ++l;
      if (l >= end)
        break;
      r = l;
      ...
      if (isDigit(originExpr[l])) {
        // 一定是数字
        while (r < end && isDigit(originExpr[r]))
          ++r;
      ...
      if (originExpr[l] == ')') {
        ...
        if (opStack.empty())
          throw std::runtime_error{"[Parser] 括号不匹配."};
        opStack.pop();
        l = r;
        continue;
      }
          ...
          if (funcs.count(curFuncName)) {
            ...
            while (!parens.empty()) {
              ...
              parsedArgs.emplace_back(
                  std::move(parse(0, r - l, originExpr.substr(l, r - l))));
              ++r;
              l = r;
            }
            if (funcs[curFuncName].isLimitedArgc() &&
                funcs[curFuncName].getArgCnt() != parsedArgs.size()) {
      ...
      if (isOp(originExpr[l])) {
        ...
        if (ops.count(originExpr[l])) {
          while (!opStack.empty() &&
                 !(ops[opStack.top()] < ops[originExpr[l]])) {
            if (opStack.top() == '(')
              break;
            rpn += opStack.top();
            rpn += ' ';
  ...
  T calc(std::size_t beg, std::size_t end, std::string_view rpnExpr) {
    ...
    while (l < rpnend) {
      while (l < rpnend && isDelim(rpnExpr[l]))
        ++l;
      if (l >= rpnend)
        break;
      r = l;
      if (isDigit(rpnExpr[l])) {
        // is digit
        while (r < rpnend && isDigit(rpnExpr[r]))
          ++r;
          ...
          if (funcs.count(curFuncName)) {
            ...
            while (!parens.empty()) {
              while (r < rpnend && !(rpnExpr[r] == ',' && parens.size() == 1)) {
                ...
                if (rpnExpr[r] == ')') {
                  if (parens.empty() || parens.top() == ')')
                    throw std::runtime_error{"[Calc] 括号不匹配."};
                  parens.pop();
                  if (parens.empty())
                    break;
                }
              ...
              parsedArgVars.emplace_back(
                  std::move(calc(0, r - l, rpnExpr.substr(l, r - l))));
              ++r;
              l = r;
            }
      ...
      if (isOp(rpnExpr[l])) {
        ...
        if (ops.count(rpnExpr[l])) {
          if (numStack.size() < 2)
            throw std::runtime_error{"[Calc] RPN 生成错误."};
          T opArg2{numStack.top()};
          numStack.pop();
          T opArg1{numStack.top()};
          numStack.pop();
    ...
    if (numStack.size() > 1)
      throw std::runtime_error{"[Calc] RPN 表达式不合法."};
    return numStack.empty() ? 0 : numStack.top();
  }

  // test only
  std::string recursiveParse(std::string_view str,
    ...
    while (l < end) {
      ...
      if (!parens.empty())
        throw std::runtime_error{"[RecParse] 括号不匹配."};
    ...
    while (r < nend) {
      while (r < nend &&
             !(r < nend - 1 && newStr[r] == '_' && isDigit(newStr[r + 1])))
        ++r;
...
public:
  RPNExprParser() {};
  RPNExprParser(std::string_view _originExpr, char _delim = ' ',
                const OpTable &_ops = {}, const VarTable &_vars = {},
                const FuncTable &_funcs = {})
      : originExpr{_originExpr}, delim{_delim}, ops{_ops}, vars{_vars},
        funcs{_funcs} {}
  RPNExprParser(const RPNExprParser &obj) = delete;
  RPNExprParser(const RPNExprParser &&obj) = delete;
  ~RPNExprParser() = default;

  void addOp(char name, const std::function<T(const T &, const T &)> &op,
             int priority) {
    if (ops.count(name))
      throw std::runtime_error{"[Options] 已存在的操作符."};
    ops.emplace(name, Operator<T>{op, priority});
  }
  void addVar(std::string_view name, const T &var) {
    if (vars.count(name))
      throw std::runtime_error{"[Options] 已存在的变量名."};
    vars.emplace(name, var);
  }
  void addFunc(std::string_view name,
               const std::function<T(const std::vector<T> &)> &func,
               size_t argCnt = 0, bool limitedArgc = false) {
    if (funcs.count(name))
      throw std::runtime_error{"[Options] 已存在的函数名."};
  ...
  std::string_view parseExpr() {
    if (rpnExpr.empty())
      rpnExpr = std::move(parse(0, originExpr.size(), originExpr));
    return rpnExpr;
  }
