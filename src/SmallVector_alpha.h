...
namespace MeowUtils {
  ...
  private:
    ...
    void destruct_in_(ValType *pos, std::uint64_t size) noexcept(std::is_nothrow_destructible_v<ValType>) {
    ...
    void expand_with_(std::ptrdiff_t offset, std::uint64_t with_size, std::uint64_t incre_factor = INCREMENT_FACTOR) {
      ...
      if (needed_capacity <= capacity_) [[likely]] {
        if (offset == size_) [[likely]] { // Fast path for `emplace_back`.
          return;
        } else {
  ...
  public:
    ...
    void resize(std::uint64_t size) {
      if (size >= size_) [[likely]] {
        ...
        shrink_without_(size_, size_ - size);
      }
